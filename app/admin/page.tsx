...
/* ========= types ========= */
type Nurse = { staff_id: number; users: { first_name: string; last_name: string } }
type Admission = {
  admission_id: number
  room_id: number
  nurse_id:     number
...
export default function AdminDashboard() {
  ...
  const [updateFeedback, setUpdateFeedback] = useState<{ ok: boolean; msg: string } | null>(null)

  type Staff = { staff_id: number; users: { first_name: string; last_name: string } }
  ...
  const [billingUpdateFeedback, setBillingUpdateFeedback] = useState<{ ok: boolean; msg: string } | null>(null)
  ...
  const [itemFeedback, setItemFeedback] = useState<{ ok: boolean; msg: string } | null>(null)
  ...
  async function handleAddItem(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // เคลียร์ฟอร์ม
        setNewItem({ billId:'', itemType:'Medicine', itemIdRef:'', description:'', quantity:'', unitPrice:'' })
  ...
  useEffect(() => {
    async function load() {
      ...
      const patientsRes = await fetch('/api/admin/patient', { credentials: 'include' })
  ...
  function handleAdmissionChange(e: React.ChangeEvent<HTMLSelectElement>) {
    setSelectedAdm(admissions.find(a => a.admission_id === +e.target.value) || null)
  }
  ...
  const [billingFeedback, setBillingFeedback] = useState<{ ok: boolean; msg: string } | null>(null)
  ...
  async function handleCreateBilling(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const json = await res.json()
      if (res.ok) {
        setBillingFeedback({ ok: true, msg: `✔ Bill created successfully (ID: ${json.bill_id})` })
        setNewBilling({ patientId: '', totalPrice: '' })
        // (ถ้าต้องรีโหลด list bills ก็เรียก fetch ใหม่ที่นี่)
      } else {
        setBillingFeedback({ ok: false, msg: json.error || 'Bill creation failed' })
      }
    } catch (err) {
      console.error(err)
  ...
  async function handleUpdateBilling(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setBillingUpdateFeedback({ ok: false, msg: json.error || 'Update failed' })
      }
    } catch (err) {
      console.error(err)
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside className="w-64 bg-white border-r p-6">
        ...
        <nav className="flex flex-col gap-4">
          {sidebarLinks.map((l) => (
            <a key={l.label} href="#" className="flex items-center gap-3 text-gray-700 hover:bg-gray-100 p-2 rounded-md">
              {l.icon}
            ...
            <div className="space-y-6">
              {/* Assign Nurse card */}
                ...
                <form className="space-y-4" onSubmit={handleAssign}>
                    ...
                    >
                      <option value="" disabled hidden>-- Select Admission --</option>
                      {admissions.map(a => (
                    ...
                    <select
                        ...
                        className="w-full border px-3 py-2 rounded-md focus:ring-2 focus:ring-blue-500"
                    >
                        <option value="" disabled hidden>-- Select Nurse --</option>
                        {nurses.map(n => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
                ...
                <section className="bg-white rounded-lg shadow p-6">
                  ...
                  <form onSubmit={handleCreateStaff} className="space-y-4">
                    ...
                    <select name="userId" required className="w-full border rounded p-2">
                      <option value="" disabled hidden>-- เลือก User --</option>
                      {userList.map(u => (
                    ...
                    <div>
                      ...
                      <select name="departmentId" required className="w-full border rounded p-2">
                    ...
                    <div>
                      ...
                      <select name="staffType" required className="w-full border rounded p-2">
                    ...
                    <div>
                      ...
                      <select name="employmentStatus" required className="w-full border rounded p-2">
                  ...
                  <form onSubmit={handleUpdateStaff} className="space-y-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={e => setUpdateData(d => ({ ...d, departmentId: e.target.value }))}
                        className="w-full border rounded p-2"
                      >
                        <option value="" disabled hidden>-- Select Department --</option>
                        <option value="1">Cardiology 1</option>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={e => setUpdateData(d => ({ ...d, staffType: e.target.value }))}
                        className="w-full border rounded p-2"
                      >
                        <option value="Doctor">Doctor</option>
                        <option value="Nurse">Nurse</option>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setUpdateData(d => ({ ...d, licenseNumber: e.target.value }))}
                        className="w-full border rounded p-2"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={e => setUpdateData(d => ({ ...d, employmentStatus: e.target.value }))}
                        className="w-full border rounded p-2"
                      >
                        <option value="Active">Active</option>
                        <option value="On_Leave">On Leave</option>
              ...
              <form onSubmit={handleCreateBilling} className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setNewBilling(b => ({ ...b, patientId: e.target.value }))}
                    required
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setNewBilling(b => ({ ...b, totalPrice: e.target.value }))}
                    required
              ...
              <form onSubmit={handleUpdateBilling} className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setUpdateBilling(b => ({ ...b, billId: e.target.value }))}
                    required
                    className="w-full border rounded p-2"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setUpdateBilling(b => ({ ...b, status: e.target.value }))}
                    required
                    className="w-full border rounded p-2"
                  >
              ...
              <form onSubmit={handleAddItem} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setNewItem(i => ({ ...i, billId: e.target.value }))}
                    className="w-full border rounded p-2"
                  >
                    <option value="" disabled>-- Select Invoice --</option>
                    {invoices.map(i => (
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setNewItem(i => ({ ...i, itemIdRef: e.target.value }))}
                    className="w-full border rounded p-2"
                  />
                </div>
