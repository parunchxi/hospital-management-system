...
export async function GET() {
  ...
  if (role !== 'Admin') {
    return NextResponse.json({ error: 'Only admins can view dashboard stats' }, { status: 403 });
  }
  ...
  try {
    ...
    const [
      ...
      supabase.from('medical_staff').select('*', { count: 'exact', head: true }).eq('staff_type', 'Doctor'),
      supabase.from('medical_staff').select('*', { count: 'exact', head: true }).eq('staff_type', 'Nurse'),
      supabase.from('medical_staff').select('*', { count: 'exact', head: true }).eq('staff_type', 'Pharmacist'),
      supabase.from('departments').select('*', { count: 'exact', head: true }),
      supabase.from('medical_records').select('*', { count: 'exact', head: true }).eq('visit_status', 'Scheduled'),
      supabase.from('medical_records').select('*', { count: 'exact', head: true }),
      supabase.from('treatments').select('*', { count: 'exact', head: true }),
      supabase.from('medicine_stock').select('*', { count: 'exact', head: true }),
      ...
      supabase
        ...
        .then(res => {
          ...
          return { data: mostActiveDoctor ? [{ doctor_id: mostActiveDoctor }] : [] };
        }),
      ...
      supabase
        ...
        .then(async (res) => {
          ...
          const doctorIds = Array.from(new Set(res.data.map(r => r.doctor_id)));
    ...
    // Calculate revenue
    const revenue = (revenueRes.data || []).reduce((sum, item) => sum + item.total_price, 0);
    ...
    const stats = [
      ...
      { label: 'Total Pharmacists', icon: 'FilePlus', value: totalPharmacists || 0 },
      { label: 'Departments', icon: 'Hospital', value: departments || 0 },
      { label: 'Appointments', icon: 'CalendarDays', value: appointments || 0 },
      { label: 'Total Records', icon: 'ClipboardList', value: totalRecords || 0 },
      ...
      { label: 'Most Active Doctor', icon: 'Activity', value: mostActiveDoctorName },
      { label: 'Most Visited Dept.', icon: 'LayoutList', value: mostVisitedDeptRes.name },
      { label: 'Visits This Month', icon: 'TrendingUp', value: visitsThisMonth || 0 },
    ];
