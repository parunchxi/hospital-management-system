...
export default function DispenseButton({
  onDispenseSuccess,
}: DispenseButtonProps) {
  ...
  const [selectedMedicineDetails, setSelectedMedicineDetails] =
  ...
  const handleDispenseMedicine = async () => {
    ...
    const selectedMedicine = medicines.find(
      (medicine) => medicine.medicine_id === medicine_id,
    )

    if (selectedMedicine && Number(quantity) > selectedMedicine.quantity) {
      setError(
        `Quantity cannot exceed available stock (${selectedMedicine.quantity}).`,
      )
      return
    }
  ...
  const filteredRecords = records.filter((record) => {
    const patientName =
      `${record.patients.users.first_name} ${record.patients.users.last_name}`.toLowerCase()
              ...
              <Card className="border-muted transition-all duration-200">
                <CardHeader className="p-4 pb-2">
                  <div className="flex justify-between items-start">
                    ...
                    <Badge
                      variant={
                        selectedRecordDetails.patient_status === 'Outpatient'
                          ? 'outline'
                          : 'default'
                      }
                      className={
                ...
                <CardContent className="p-4 pt-0">
                  <div className="grid md:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <Badge
                        variant={
                          selectedRecordDetails.visit_status === 'Scheduled'
                            ? 'secondary'
                            : selectedRecordDetails.visit_status === 'Completed'
                              ? 'success'
                              : 'default'
                        }
                      >
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {new Date(
                          selectedRecordDetails.visit_date,
                        ).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  <div className="mt-3">
                    ...
                    <p className="text-sm bg-gray-50 p-2 rounded border mt-1">
                      {selectedRecordDetails.symptoms}
                    </p>
                  </div>
                </CardContent>
              </Card>
                ...
                <SelectContent>
                  <ScrollArea className="h-[200px]">
                    {medicines.map((medicine) => (
                      ...
                      >
                        {medicine.name} ({medicine.quantity} {medicine.unit}s
                        available)
                      </SelectItem>
                    ))}
                  </ScrollArea>
                ...
                <CardContent className="p-4 pt-1">
                  ...
                  <div className="grid md:grid-cols-2 gap-3">
                    <div>
                      ...
                      <p className="font-medium flex items-center gap-1">
                        ...
                        <span className="text-sm text-muted-foreground">
                          {selectedMedicineDetails.unit}s
                        </span>
                        {selectedMedicineDetails.quantity <=
                          selectedMedicineDetails.min_stock_level && (
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {selectedMedicineDetails.supplier}
                      </p>
                    </div>
                  </div>
                </CardContent>
