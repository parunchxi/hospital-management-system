...
import { PillIcon, Calendar, PackageIcon, AlertCircle, CheckCircle2, AlertTriangle } from "lucide-react";
            ...
            <TooltipProvider key={med.medicine_id}>
              <Card 
                className={`overflow-hidden transition-all duration-200 hover:shadow-md ${
                  status === "out-of-stock" ? "border-red-200" :
                  status === "low-stock" ? "border-yellow-200" : ""
                }`}
              >
                <CardHeader className="p-4 pb-2">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="flex items-center gap-1 text-xs">
                          <Calendar className={`h-3.5 w-3.5 ${isExpiringSoon ? "text-red-500" : "text-gray-500"}`} />
                          <span className={isExpiringSoon ? "text-red-600 font-medium" : "text-gray-500"}>
                            {new Date(med.expiry_date).toLocaleDateString()}
                          </span>
                        </div>
                      </TooltipTrigger>
                      <TooltipContent side="top">
                        <p>{isExpiringSoon ? "Expiring Soon" : "Expiry Date"}</p>
                      </TooltipContent>
                    </Tooltip>
                  </div>
