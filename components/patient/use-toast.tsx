...
function ToastComponent({
  ...
  return (
    <div
      className={`${bgColor} text-white p-4 rounded shadow-md min-w-[300px] animate-in fade-in slide-in-from-right`}
    >
...
function renderToasts(toasts: React.ReactNode[]) {
  ...
  if (toastRoot) {
    toastRoot.render(<React.StrictMode>{toasts}</React.StrictMode>)
  }
}
